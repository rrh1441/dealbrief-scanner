...
DELIVERABLE FORMAT  
Generate clean, well-formatted HTML with proper styling. Use semantic HTML elements and inline CSS for professional appearance.

Structure:
1. **Executive Snapshot** – ≤150 words summarising the overall risk posture plus a 3-color (Green / Yellow / Red) *overall* rating.  
...
- Bullet points using <ul>/<li>
- Citations as superscript links
- Color-coded risk ratings (green/yellow/red badges)
- Clean typography and spacing`;

const MODELS = [
  { name: 'o4-mini-2025-04-16', provider: 'openai', displayName: 'OpenAI o4-mini' },
  { name: 'claude-sonnet-4-20250514', provider: 'claude', displayName: 'Claude Sonnet 4' }
];
...
async function callOpenAI(model, prompt, data) {
  ...
  const completion = await openai.chat.completions.create({
    model: model,
    messages: [
      { role: "system", content: DEALBRIEF_PROMPT },
...
async function generatePDF(htmlContent, modelName, companyName, scanId) {
  const filename = `/tmp/${companyName.replace(/[^a-zA-Z0-9]/g, '_')}_${modelName.replace(/[^a-zA-Z0-9]/g, '_')}_${scanId}_${Date.now()}.pdf`;
...
<head>
  <meta charset="UTF-8">
  <title>DealBrief Security Assessment - ${companyName}</title>
  <style>
    body {
